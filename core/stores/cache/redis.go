...
func (c redisNode) prefixKey(key string) string {
	if c.cachePrefix != "" {
...
func (c redisNode) GetPrefixKeysCtx(ctx context.Context, keyPrefix string) ([]string, error) {
	keyPrefix = c.prefixKey(keyPrefix)
...
func (c redisNode) Del(keys ...string) error {
	var withPrefixKeys []string
	for _, v := range keys {
		withPrefixKeys = append(withPrefixKeys, c.prefixKey(v))
	}
	return c.node.Del(withPrefixKeys...)
}
...
func (c redisNode) DelCtx(ctx context.Context, keys ...string) error {
	...
	return c.node.DelCtx(ctx, withPrefixKeys...)
}
