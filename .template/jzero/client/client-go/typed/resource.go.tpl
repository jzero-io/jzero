{{define "methodDefine"}}{{.MethodName}}({{if or .IsStreamServer .IsStreamClient .IsSpecified}}{{else}}ctx context.Context,{{end}}param *{{.RequestBody.Package | base}}.{{.RequestBody.Name}}) ({{if or .IsStreamServer .IsStreamClient .IsSpecified}}*rest.Request{{else}}*{{.ResponseBody.Package | base}}.{{.ResponseBody.Name}}{{end}}, error){{end}}
...
{{range $k, $v := .HTTPInterfaces}}func (x *{{$.Resource}}Client) {{template "methodDefine" $v}} {
	...
	return request, nil{{else}}var resp {{$v.ResponseBody.Package | base}}.{{$v.ResponseBody.Name}}
